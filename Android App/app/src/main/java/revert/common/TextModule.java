...
public class TextModule {
    ...
    public static TextPaint getTextPaint() {
        if (textPaint == null) textPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG);
        textPaint.setColor(defaultTextColor);
        return textPaint;
    }

    public static TextPaint getTextPaint(int color) {
        if (textPaint == null) {
    ...
    public static TextPaint getTextPaint(int color, Typeface typeface, boolean reset) {
        if (textPaint == null || reset) {
            textPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG);
            //textPaint.setLinearText(true);
    ...
    private static RectF DrawScaledText(Canvas canvas, String value, float left, float top, float width, float height, Paint paint,
                                        Layout.Alignment hAlignment, Layout.Alignment vAlignment, boolean strokeText) {
        float textSize = CalculateTextSize(value, paint, width, height);
        return DrawText(canvas, value, left, top, width, height, textSize, paint, hAlignment, vAlignment, strokeText);
    }

    public static RectF DrawText(Canvas canvas, String value, float left, float top, float width, float height, float textSize, Paint paint,
                                 Layout.Alignment hAlignment, Layout.Alignment vAlignment, boolean strokeText) {
        ...
        switch (vAlignment) {
            case ALIGN_CENTER:
                y = top + height / 2f + textRect.height() / 2f - textRect.bottom;
                break;
            case ALIGN_OPPOSITE:
                y = top + height;
                break;
            default:
                y = top - (fontMetrics.top + fontMetrics.ascent) / 2f - fontMetrics.bottom;
                //y = top - fontMetrics.ascent;
                break;
        }

        if (strokeText) {
            paint.setStrokeWidth(Math.max(textSize / 20f, 2f));
        ...
        //canvas.drawRect(renderedTextRect, paint);
